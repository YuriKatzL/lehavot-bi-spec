...
>
> לעשות לפי הלוגיקה של דוח 740, או שראובן יחליט על גישה אחרת?
...
| % from Budget | אחוז ביצוע מתקציב | מכירות בפועל / תקציב × 100 | ❓ Excel או מסך? |
| % from LY | אחוז משנה שעברה | מכירות השנה / מכירות אשתקד × 100 | ❓ מסך תעודות |
...
|---|-------|-------|----------------|
| 1 | BU | סגמנט עסקי | ❓ נדרש מיפוי מסך |
| 2 | Current Week | מכירות השבוע | ❓ מסך תעודות — שדה סכום |
| 3 | MTD Sales | מכירות מתחילת החודש | ❓ מסך תעודות — שדה סכום |
| 4 | MTD Budget | תקציב יחסי לתקופה | ❓ Excel או מסך? |
| 5 | Full Month Budget | תקציב מלא לחודש | ❓ Excel או מסך? |
| 6 | MTD % from Budget | אחוז ביצוע | חישוב: עמודה 3 / עמודה 4 |
| 7 | MTD Last Year | אותה תקופה אשתקד | ❓ מסך תעודות — שנה קודמת |
| 8 | MTD % from LY | אחוז משנה שעברה | חישוב: עמודה 3 / עמודה 7 |
...
|-----|-------|-------|----------------|
| % Time Elapsed | אחוז הזמן שעבר בתקופה | יום נוכחי / ימים בתקופה × 100 | חישוב Power BI |
| % from LY | אחוז משנה שעברה | הזמנות השנה / הזמנות אשתקד × 100 | ❓ מסך הזמנות |
...
|---|-------|-------|----------------|
| 1 | BU | סגמנט עסקי | ❓ נדרש מיפוי מסך |
| 2 | Current Week | הזמנות השבוע | ❓ מסך הזמנות — שדה סכום |
| 3 | MTD Orders | הזמנות מתחילת החודש | ❓ מסך הזמנות — שדה סכום |
| 4 | MTD Last Year | אותה תקופה אשתקד | ❓ מסך הזמנות — שנה קודמת |
| 5 | MTD % vs LY | אחוז משנה שעברה | חישוב: עמודה 3 / עמודה 4 |
| 6 | QTD Orders | הזמנות מתחילת הרבעון | ❓ מסך הזמנות — שדה סכום |
| 7 | QTD Last Year | אותה תקופה אשתקד | ❓ מסך הזמנות — שנה קודמת |
...
| # | עמודה | תיאור | מקור (מסך.שדה) |
|---|-------|-------|----------------|
| 1 | Due Date | תאריך אספקה צפוי | ❓ מסך הזמנות — שדה תאריך אספקה |
| 2 | Open Orders | סכום הזמנות פתוחות | ❓ מסך הזמנות — פילטר לפי סטטוס |
| 3 | Distributor | מפיץ | ❓ מסך סוכנים — שם סוכן |

> **❓ שאלה #2 לראובן:** במסך הזמנות — איזה שדה מציין סטטוס פתוח/סגור?
...
### 6.2 מסכים נדרשים (OData Entities)

> **הערה:** Priority OData עובד עם מסכים (forms), לא טבלאות ישירות.

| מסך | שימוש | גישה OData |
|-----|-------|------------|
| `DOCUMENTS_D` | תעודות משלוח — נתוני מכירות | ❓ לבדוק |
| `ORDERS` | נתוני הזמנות | ❓ לבדוק |
| `CUSTOMERS` | לקוחות — מיפוי לסגמנט | ❓ לבדוק |
| `AGENTS` | סוכנים / מפיצים | ❓ לבדוק |
| `FAMILY` | משפחות מוצרים | ❓ לבדוק |
| `Excel / ???` | נתוני תקציב | ❓ Excel או מסך בפריוריטי? |
...
> **❓ שאלה #3 לראובן:** איזה מסך להשתמש ב-OData?
> - `DOCUMENTS_D` (תעודות משלוח)?
> - `INVOICES` (חשבוניות)?
> - מסך אחר?
>
> האם דוח 740 משתמש במסך ספציפי?
...
| # | שאלה | אחראי | סטטוס |
|---|------|-------|-------|
| 1 | מיפוי סגמנטים — איזה שדה/מסך בפריוריטי? | ראובן | ❓ פתוח |
| 2 | סטטוס הזמנה — איזה שדה במסך ORDERS? | ראובן | ❓ פתוח |
| 3 | מסך מקור למכירות — DOCUMENTS_D / INVOICES / אחר? | ראובן | ❓ פתוח |
| 4 | מקור תקציב — Excel או מסך בפריוריטי? מבנה? | ראובן | ❓ פתוח |
